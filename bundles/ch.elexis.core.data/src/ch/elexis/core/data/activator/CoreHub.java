...
public class CoreHub implements BundleActivator {
	...
	public static final String APPLICATION_NAME = "Elexis Ungrad"; //$NON-NLS-1$
	 ...
	 * property ${pom.version} by the current build version and place it into the
	 * file /version.properties of each jar file.
	 * 
		...
		try {
			...
			if (n == 0) {
				MessageEvent.fireLoggedError("Too many instances",
						"Too many concurrent instances of Elexis running. Check elexislock files in " + userDir);
	...
	public static void logoffAnwender() {
		...
		ElexisEventDispatcher.getInstance().fire(new ElexisEvent(null, User.class, ElexisEvent.EVENT_DESELECTED));
