...
public class Fall extends PersistentObject implements IFall, ITransferable<FallDTO> {
	...
	public static final String FLD_BILLING = "billing";
	...
	// @formatter:off
	static {
		addMapping(TABLENAME, FLD_PATIENT_ID, FLD_RES + "=Diagnosen", //$NON-NLS-1$
				FLD_DATUM_VON + "=S:D:DatumVon", // $NON-NLS-2$
				FLD_DATUM_BIS + "=S:D:DatumBis", // $NON-NLS-2$
				FLD_GARANT_ID, FLD_BEHANDLUNGEN + "=LIST:FallID:BEHANDLUNGEN:Datum", // $NON-NLS-2$
				FLD_BEZEICHNUNG, FLD_GRUND, FLD_BILLINGSYSTEM, FLD_KOSTENTRAEGER + "=KostentrID", // $NON-NLS-2$
				FLD_VERS_NUMMER, FLD_FALL_NUMMER, FLD_RN_PLANUNG + "=BetriebsNummer", //$NON-NLS-1$
				FLD_EXTINFO);
	 ...
	 * 
	 * @param force delete even if KOnsultationene xist (in that case, all
	...
	public static String getDefaultCaseLabel() {
		return CoreHub.userCfg.get(Preferences.USR_DEFCASELABEL, Preferences.USR_DEFCASELABEL_DEFAULT);
	}

	public static String getDefaultCaseReason() {
		return CoreHub.userCfg.get(Preferences.USR_DEFCASEREASON, Preferences.USR_DEFCASEREASON_DEFAULT);
	}

	public static String getDefaultCaseLaw() {
		return CoreHub.userCfg.get(Preferences.USR_DEFLAW, BillingSystem.getAbrechnungsSysteme()[0]);
	}
