...
public class Leistungscodes extends PreferencePage implements IWorkbenchPreferencePage {
	...
	List<IConfigurationElement> list_RnOutputters = Extensions
	...
	protected Control createContents(final Composite parent) {
		...
		SWTHelper.createHyperlink(ret, Messages.Core_New_ellipsis, new HyperlinkAdapter() {
			@Override
		...
		SWTHelper.createHyperlink(ret, Messages.Core_Delete_ellipsis, new HyperlinkAdapter() {
			@Override
			public void linkActivated(HyperlinkEvent e) {
				TableItem sel = table.getSelection()[0];
				String bName = sel.getText(0);
				if (SWTHelper.askYesNo(MessageFormat.format(Messages.Leistungscodes_reallyDelete, bName),
		...
		bCheckZero
				.setSelection(configService.getActiveUserContact(Preferences.LEISTUNGSCODES_BILLING_ZERO_CHECK, false));
		...
		bStrictCheck.setSelection(configService.getActiveUserContact(Preferences.LEISTUNGSCODES_BILLING_STRICT, true));
		...
		bOptify.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
				configService.setActiveUserContact(Preferences.LEISTUNGSCODES_OPTIFY, bOptify.getSelection());
			}

		});
		bOptify.setSelection(configService.getActiveUserContact(Preferences.LEISTUNGSCODES_OPTIFY, true));
		...
		bAllowOverrideStrict.setSelection(
				configService.getActiveUserContact(Preferences.LEISTUNGSCODES_ALLOWOVERRIDE_STRICT, false));
		...
		bOptifyXray.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
				configService.setActiveUserContact(Preferences.LEISTUNGSCODES_OPTIFY_XRAY, bOptifyXray.getSelection());
			}

		});
		bOptifyXray.setSelection(configService.getActiveUserContact(Preferences.LEISTUNGSCODES_OPTIFY_XRAY, true));
		...
		bObligation.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
				configService.setActiveUserContact(Preferences.LEISTUNGSCODES_OBLIGATION, bObligation.getSelection());
			}

		});
		bObligation.setSelection(configService.getActiveUserContact(Preferences.LEISTUNGSCODES_OBLIGATION, false));
		...
		bRemoveOpenReminders.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
				configService.set(Preferences.RNN_REMOVE_OPEN_REMINDER, bRemoveOpenReminders.getSelection());
			}
	 ...
	 */
	public void reload() {
		List<String> systeme = configService.getSubNodes(Preferences.LEISTUNGSCODES_CFG_KEY, true);
	...
	class AbrechnungsTypDialog_InputDialog extends Dialog {
		...
		/**
		 *
		 * @param parentShell        Shell, the parentShell...
		 * @param dialogTitle        String, the text displayed in the window bar
		 * @param dialogMessage      String, the message displayed at the top of the
		 *                           window
		 * @param initialValue       String, the value displayed for the name of the
		 *                           field
		 * @param noDuplicatesList   ListDisplay<String>[], array of ListDisplay's
		 *                           containing item names which should not be
		 *                           duplicated
		 * @param text               String, the initial text value for the text editor
		 *                           field (for the item values)
		 * @param isNumericChecked   Boolean, should the checkbox be checked
		 * @param isStyledChecked    Boolean, should the checkbox be checked
		 * @param isMultilineChecked Boolean, should the checkbox be checked
		 * @param changeTypeItems    String[], list of types to which this field can be
		 *                           changed to - if null, then display no combo
		 */
		public AbrechnungsTypDialog_InputDialog(Shell parentShell, final String dialogTitle, final String dialogMessage,
				final String initialValue, ListDisplay<String>[] noDuplicatesList, String text,
				boolean isNumericChecked, boolean isStyledChecked, boolean isMultilineChecked,
				String[] changeTypeItems) {
			super(parentShell);
			...
			if (changeTypeItems != null) {
				for (int i = 0; i < changeTypeItems.length; i++) {
					changeTypeItems[i] = changeTypeItems[i].replaceAll("\\.\\.\\.", StringTool.leer); //$NON-NLS-1$
				}
			}
		...
		protected void calcFieldPresence() {
			...
			if (cCurrentFieldType.equalsIgnoreCase(Messages.Leistungscodes_contactHL)) {
			} else if (cCurrentFieldType.equalsIgnoreCase(Messages.Leistungscodes_textHL)) {
				cHasStyledCheckbox = true;
				cHasMultilineCheckbox = true;
			} else if (cCurrentFieldType.equalsIgnoreCase(Messages.GlobalActions_Redate)) {
			} else if (cCurrentFieldType.equalsIgnoreCase(Messages.Leistungscodes_comboHL)) {
				cHasTextEditor = true;
				cHasNumericCheckbox = true;
			} else if (cCurrentFieldType.equalsIgnoreCase(Messages.Core_List_ellipsis)) {
				cHasTextEditor = true;
				cHasNumericCheckbox = true;
		...
		protected void okPressed() {
			...
			result[1] = (tTextEditor == null || tTextEditor.isDisposed()) ? StringTool.leer : tTextEditor.getText();
			...
			if ((result[0].indexOf(ARGUMENTSSDELIMITER) >= 0) || (result[0].indexOf(DEFINITIONSDELIMITER) >= 0)
					|| (result[0].indexOf(ITEMDELIMITER) >= 0)) {
				errorString = errorString + Messages.Leistungscodes_ErrorNameNoSpecialChars;
			}
			if ((!cInitialValue.equalsIgnoreCase(result[0])) && (fieldExistsAlready(result[0], cNoDuplicatesList))) {
				errorString = errorString + Messages.Leistungscodes_ErrorFieldAlreadyExists;
			}
			if (cHasTextEditor) {
				if (cTextNeeded) {
					if ((result[1].length() > 4) && (result[1].substring(0, 4).equalsIgnoreCase("SQL:"))) { //$NON-NLS-1$
						// *** SQL-variant, is directly passed through
					} else if ((result[1].length() > 7) && (result[1].substring(0, 7).equalsIgnoreCase("SCRIPT:"))) { //$NON-NLS-1$
						...
						if (!tmp.isEmpty()) {
							if ((tmp.substring(0, 1).equalsIgnoreCase(DEFINITIONSDELIMITER))
								...
								errorString = errorString + Messages.Leistungscodes_ErrorNoEmptyItemsAllowed;
							}
						}
						if ((result[1].indexOf(ARGUMENTSSDELIMITER) >= 0)
								|| (result[1].indexOf(DEFINITIONSDELIMITER) >= 0)
								|| (result[1].indexOf(ITEMDELIMITER) >= 0)) {
							errorString = errorString + Messages.Leistungscodes_ErrorItemsNoSpecialChars;
						}
					}
				}
			}
	...
	class AbrechnungsTypDialog extends TitleAreaDialog {
		 ...
		 *
		 * @param shell
		 * @param abrdef String, the name of the billing system
		 */
		...
		protected Control createDialogArea(final Composite parent) {
			...
			new Label(upperPartComp, SWT.NONE).setText(Messages.Core_Name);
			...
			cbReason.setInput(new String[] { FallConstants.TYPE_DISEASE, FallConstants.TYPE_ACCIDENT,
					FallConstants.TYPE_MATERNITY, FallConstants.TYPE_PREVENTION, FallConstants.TYPE_BIRTHDEFECT,
					FallConstants.TYPE_OTHER });
			...
			// *** label/combo for law
			new Label(upperPartComp, SWT.NONE).setText(Messages.Core_Law_Name);
			...
			lbTaxp.setText(Messages.Core_Multiplicator);
			...
			bUseMultiForEigenleistung.setText(Messages.Leistungscodes_useMultiplierForCustomServices);
			bUseMultiForEigenleistung.setSelection(MultiplikatorList.isEigenleistungUseMulti(tName.getText()));
			...
			if (AccessControlServiceHolder.get().evaluate(EvACEs.CASE_DEFINE_SPECIALS)) {
				...
				fdUnused.addMoveToAction(ldRequirements, Messages.Leistungscodes_moveItemToRequiredData,
		...
		private void removeRequiredStringGesetzFromFallExtInfo() {
			...
			if (selection != null && StringUtils.containsIgnoreCase(selection, "Gesetz")) { //$NON-NLS-1$
				...
				String message = MessageFormat.format(
						...
						split[0]);
				boolean performDelete = MessageDialog.openQuestion(UiDesk.getTopShell(), "Remove from Faelle", message);
				if (performDelete) {
		...
		public void create() {
			...
			getShell().setText(Messages.Invoice_System);
		}
	 ...
	 * @return
	 */
	private boolean fieldExistsAlready(final String fieldName, ListDisplay<String>... ldNoDuplicates) {
	 ...
	 * below there is a ListDisplay on the left side and a navigator on the right
	 * side. <br>
	...
	class FieldDefsDisplay extends Composite {
		...
		/**
		 * set the noDuplicatesList value, which contains the Lists containing items not
		 * to be duplicated for the "move to other list action"
		 *
		 * @param listArray
		 */
		...
		/**
		 * set the noDuplicatesList value, which contains the Lists containing items not
		 * to be duplicated for the "create new field action"
		 *
		 * @param listArray
		 */
		...
		public void addMoveToAction(ListDisplay<String> destinationList, String toolTipText,
			...
			if (toolBar != null) {
				...
				ListPopUpMenuAction newAction = new ListPopUpMenuAction(toolTipText, imageDescriptor, toolTipText,
						newItemIx, destinationList, enabled);
		...
		private FieldDefsDisplay(Composite parent, int style, String[] listItems) {
			...
			listDisplay.addHyperlinks(Messages.Leistungscodes_contactHL, Messages.Leistungscodes_textHL,
			...
			addToolItem(toolBar, Images.IMG_ARROWUP.getImage(), Messages.Leistungscodes_moveItemUp, MOVEITEMUP, true);
			addToolItem(toolBar, Images.IMG_ARROWDOWN.getImage(), Messages.Leistungscodes_moveItemDown, MOVEITEMDOWN,
					true);
			addToolItem(toolBar, null, Messages.Leistungscodes_moveItemDown, -1, false);
			addToolItem(toolBar, Images.IMG_REMOVEITEM.getImage(), Messages.Leistungscodes_deleteItem, DELETEITEM,
					true);
			...
			ListPopUpMenuAction moveItemUpAction = new ListPopUpMenuAction(Messages.Leistungscodes_moveItemUp,
					Images.IMG_ARROWUP.getImageDescriptor(), Messages.Leistungscodes_moveItemUp, MOVEITEMUP,
					listDisplay, true);
			ListPopUpMenuAction moveItemDownAction = new ListPopUpMenuAction(Messages.Leistungscodes_moveItemDown,
					Images.IMG_ARROWDOWN.getImageDescriptor(), Messages.Leistungscodes_moveItemDown, MOVEITEMDOWN,
					listDisplay, true);
			ListPopUpMenuAction delItemAction = new ListPopUpMenuAction(Messages.Core_Delete,
					Images.IMG_REMOVEITEM.getImageDescriptor(), Messages.Leistungscodes_removeConstraintTT, DELETEITEM,
					listDisplay, true);
			ListPopUpMenuAction changeItemAction = new ListPopUpMenuAction(Messages.Leistungscodes_editItem,
					Images.IMG_EDIT.getImageDescriptor(), Messages.Leistungscodes_editItem, EDITITEM, listDisplay,
					true);
			...
			if (child != null) {
				...
				child.addKeyListener(new KeyListener() {
					...
					// #6105
					private void moveCostBearerFromExtInfoToDBRow(org.eclipse.swt.widgets.List child) {
						...
						if (selection != null && selection.length == 1) {
							...
							if ("K".equals(fieldType.substring(0, 1)) && fieldName != null && getData() != null) { //$NON-NLS-1$
								...
								if (performMove) {
									BusyIndicator.showWhile(UiDesk.getDisplay(), () -> {
										BillingSystem.moveCostBearerFromExtinfoToDBRow((String) getData(), fieldName);
										deleteListItem();
									});
								}
							}
		 ...
		 *
		 * @param parent      the toolBar/navigator
		 * @param image       the image to be used
		 * @param toolTipText the tooltip to be shown
		 * @param actionType  the action type (MOVEITEMUP, MOVEITEMDOWN, DELETEITEM,
		 *                    EDITITEM, LASTFIXEDITEM, ...)
		...
		private org.eclipse.swt.widgets.List getListPart(ListDisplay<String> listDisplay) {
			...
			for (int li = 0; li < children.length; li++) {
				...
				if (child.getClass().toString().equalsIgnoreCase("class org.eclipse.swt.widgets.List")) { //$NON-NLS-1$
		 ...
		 *
		 * @param step positive for moving down, negative for moving up
		 */
		...
		public void deleteListItem() {
			...
			if (sel == null) {
				...
				if (SWTHelper.askYesNo(StringTool.leer, Messages.Leistungscodes_reallyWantToDeleteItem)) {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				default:
					if (type > LASTFIXEDITEM) {
						moveItemToOtherList(listDisplay, moveTo_DestinationLists.get(type - LASTFIXEDITEM - 1),
								noDuplicatesList);
					}
					break;
				}
			...
			public void run() {
				...
				default:
					if (actionType > LASTFIXEDITEM) {
						moveItemToOtherList(listDisplay, moveTo_DestinationLists.get(actionType - LASTFIXEDITEM - 1),
								noDuplicatesList);
					}
					break;
				}
		...
		/**
		 *
		 * @param l                if creating a field, then this contains the field
		 *                         type name
		 * @param fieldToBeChanged if changing a field, then this contains the full
		 *                         definition for the field to be changed
		 *                         [fieldName]:[fieldType]:[itemsList]
		 */
		public void _FieldsHyperlinkActivated(final String l, String fieldToBeChanged) {
			...
			String[] changeItems = { Messages.Leistungscodes_contactHL, Messages.Leistungscodes_textHL,
					Messages.GlobalActions_Redate, Messages.Leistungscodes_comboHL, Messages.Core_List_ellipsis,
					Messages.Leistungscodes_checkboxHL, Messages.Leistungscodes_radioHL };
		...
		/**
		 * moving items from one list to another list the moved item MUST NOT be in the
		 * destination list - will not be allowed the moved item MUST NOT be in one of
		 * the noDuplicatesDisplays list - will not be allowed
		 *
		 * @param listDisplay          the current list from which the item should be
		 *                             moved away
		 * @param destDisplay          the destination list to which the item should be
		 *                             moved to
		 * @param noDuplicatesDisplays an array of ListDisplay's which contain the
		 *                             values which should not be duplicated
		 */
		@SuppressWarnings("unchecked")
		public void moveItemToOtherList(ListDisplay<String> listDisplay, ListDisplay<String> destDisplay,
				ListDisplay<String>... noDuplicatesDisplays) {
			// no duplicates allowed for destination list
			String sel = listDisplay.getSelection();
			if (sel.isEmpty())
				return;
			java.util.ArrayList<String> allItems = (ArrayList<String>) ((ArrayList<String>) destDisplay.getAll())
					.clone();
			...
			if (allItems.contains(sel.split(ARGUMENTSSDELIMITER)[0])) {
				SWTHelper.alert(StringTool.leer, Messages.Leistungscodes_definitionAlreadyExistsInDestination);
				return;
			}

			// *** no duplicates in noDuplicatesDisplays
			for (ListDisplay<String> noDuplicatesList : noDuplicatesDisplays) {
				java.util.ArrayList<String> allItems2 = (ArrayList<String>) ((ArrayList<String>) noDuplicatesList
						.getAll()).clone();
				...
				if (allItems2.contains(sel.split(ARGUMENTSSDELIMITER)[0])) {
					SWTHelper.alert(StringTool.leer, Messages.Leistungscodes_definitionAlreadyExistsSomewhere);
					return;
				}
			}
