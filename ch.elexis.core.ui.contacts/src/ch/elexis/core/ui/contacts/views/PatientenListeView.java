 ...
 * 
 */
public class PatientenListeView extends ViewPart
		...
		viewer.addDoubleClickListener(new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event){
				PropertyDialogAction pdAction =
					new PropertyDialogAction(new SameShellProvider(parent),
	...
	private void collectUserFields(){
		...
		if (CoreHub.userCfg.get(Preferences.USR_PATLIST_SHOWDOB, true)) {
			fields.add(
				Patient.BIRTHDATE + Query.EQUALS + Messages.PatientenListeView_PatientBirthdate); // $NON-NLS-1$
		}
			...
			public void doRun(){
				...
				if (ped.open() == Dialog.OK) {
					...
					for (int j = 0; j < currentUserFields.length; j++) {
						...
						} else if (current.startsWith(Patient.FLD_FIRSTNAME)
							&& pat.getVorname() != null) {
						...
						for (int i = 0; i < sel.length; i++) {
							...
							if (k.istPerson()) {
								...
								if (!StringTool.isNothing(thisAddressFLD_COUNTRY)) {
									selectedPatInfosText.append(
										"," + StringTool.space + thisAddressFLD_COUNTRY + "-");
								}
								...
								if (!StringTool.isNothing(thisAddressFLD_MOBILEPHONE)) {
									...
									selectedPatInfosText
										.append("," + StringTool.space + Kontakt.FLD_MOBILEPHONE
								...
								if (!StringTool.isNothing(thisAddressFLD_FAX)) {
									...
									// Without a colon after the label:
									selectedPatInfosText.append("," + StringTool.space
										+ Kontakt.FLD_FAX + StringTool.space + thisAddressFLD_FAX);
								}
